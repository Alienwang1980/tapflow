# Tapflow —— 把 iPad 变成 Mac 的辅助生产工具

> 📌 [待补充:新版本变更亮点 —— 把变更清单给 Claude 后插入到这里]

键盘是上个世纪创造出来用于打字的通用工具,大多数情况下它做得很好。但电脑的操作的演变，我们需要使用的键盘需要有一些新的角色，例如大量高频动作——切换窗口、调音频、启动应用——这些逐步演变为快捷键，大多都是组合键，而用快捷键或鼠标多级菜单并不总是最直观的。加上随着个性化需求的提升，这些快捷键还可以进行定制，但是键盘却不能改变，矛盾就显而易见了，这个已被习以为常和习惯忍受的问题，对于大多数人来说，这是一个很高的学习门槛
...
如今的手机能认得你的脸，电脑可以认得你的指纹，但你还在背快捷键来操作电脑，你的大脑地球上最强的运算能力，用来背快捷键简直是资源的浪费

然而情况也有变化

如今 AI 时代,Vibe Code 对对键盘的需求反而降低了，对输入方式也提出了新需求,市面上也出现了相应的产品,例如 Worklouder Creator Micro 2:
...
这类产品它们仍是实体按键:按键可以自定义,样子和大小却不会变,自定义多了以后,记住每个键的用途依然是个负担。
...
> 发布提示:2libra(2libra.com)邮箱注册即用;支持 Markdown+图片链接;新账号先正常参与几天再发帖,推广帖可能被移入"黑洞";发帖后 AI 自动选节点,可手动改。
