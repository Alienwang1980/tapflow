  ...
  try{
    ...
    if(!r.ok){
      ...
      try{
        ...
        var match=list.profiles?.find(function(p){
          return p.filename===fn||p.filename.toLowerCase()===fn.toLowerCase()||
                 p.profileName===fn||p.profileName===clean||
                 p.filename.replace(/\.json$/i,"").toLowerCase()===clean.toLowerCase()||
                 p.profileName.toLowerCase()===clean.toLowerCase();
        });
      ...
      if(!r||!r.ok){
        try{
          ...
          var bundled=await bundledR.json();
          var bMatch=bundled.profiles?.find(function(p){
            return p.filename===fn||p.filename.toLowerCase()===fn.toLowerCase()||
                   p.profileName===fn||p.profileName===clean||
                   p.filename.replace(/\.json$/i,"").toLowerCase()===clean.toLowerCase()||
                   p.profileName.toLowerCase()===clean.toLowerCase();
          ...
          if(bMatch){
            var impR=await fetch("/api/bundled-profiles/"+encodeURIComponent(bMatch.filename),{method:"POST"});
            if(impR.ok){var impD=await impR.json();fn=impD.filename||bMatch.filename;r=await fetch("/api/profiles/"+fn)}
          }
        }catch(_e2){}
      }
      if(!r||!r.ok){_showProfilePopup(activeProfile);return}
    }
