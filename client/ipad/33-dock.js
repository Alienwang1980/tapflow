...
}// ── Dock Panel — horizontal scroll, icons ──
  ...
  for (var i = 0; i < apps.length; i++) {
    ...
    if (img && img.complete) {
      ...
      if (!canvas._dockIcons[a.name]) {
        var img2 = new Image();
        img2.onload = function() { canvas._dockIcons[a.name] = img2; _drawDockGrid(canvas, apps); };
...
function _drawDockGrid(canvas, apps) {
  // forwarding to avoid duplication — defined above
}
