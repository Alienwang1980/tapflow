...
function _winCtrlXY(e,canvas){var rect=canvas.getBoundingClientRect();return{x:((e.touches?e.touches[0].clientX:e.clientX)-rect.left)*(canvas._logW/rect.width),y:((e.touches?e.touches[0].clientY:e.clientY)-rect.top)*(canvas._logH/rect.height)}}
...
function _drawWinShortcuts(canvas,pressed){var ctx=canvas.getContext("2d"),w=canvas._logW,h=canvas._logH,m=Math.min(w,h);ctx.clearRect(0,0,w,h);ctx.fillStyle=canvas._keyColor||"#1a2a2a";ctx.fillRect(0,0,w,h);var norm=canvas._winCtrlColor||"#cfd8dc",hi="#4ade80";_winClickBtns(canvas).forEach(function(b){var r=b.r,on=(pressed===b.zone),br=Math.min(r.w,r.h)*0.16;ctx.fillStyle=on?"rgba(74,222,128,0.28)":"rgba(255,255,255,0.08)";_roundRect(ctx,r.x,r.y,r.w,r.h,br);ctx.fill();ctx.strokeStyle=on?hi:"rgba(255,255,255,0.18)";ctx.lineWidth=Math.max(1,m*0.008);_roundRect(ctx,r.x,r.y,r.w,r.h,br);ctx.stroke();_winIcon(ctx,b.dir,r.x+r.w/2,r.y+r.h/2,Math.min(r.w,r.h)*0.28,on?hi:norm,m)})}
...
function _winSwipeGeom(canvas){var w=canvas._logW,h=canvas._logH,m=Math.min(w,h);return{cx:w/2,cy:h/2,R:m*0.40,knobR:m*0.17,maxR:m*0.30,m:m}}
function _drawWinSwipe(canvas,kx,ky,active){var ctx=canvas.getContext("2d"),w=canvas._logW,h=canvas._logH,g=_winSwipeGeom(canvas),cx=g.cx,cy=g.cy,norm=canvas._winCtrlColor||"#cfd8dc",hi="#4ade80";if(kx===undefined)kx=cx;if(ky===undefined)ky=cy;ctx.clearRect(0,0,w,h);ctx.fillStyle=canvas._keyColor||"#1a2a2a";ctx.fillRect(0,0,w,h);ctx.beginPath();ctx.arc(cx,cy,g.R,0,Math.PI*2);ctx.fillStyle="rgba(255,255,255,0.05)";ctx.fill();ctx.strokeStyle="rgba(255,255,255,0.18)";ctx.lineWidth=Math.max(1,g.m*0.008);ctx.beginPath();ctx.arc(cx,cy,g.R,0,Math.PI*2);ctx.stroke();var ar=g.R*0.80,as=g.m*0.05,pale=_hexToRgba(norm,0.5)||norm;_winIcon(ctx,"up",cx,cy-ar,as,pale,g.m);_winIcon(ctx,"down",cx,cy+ar,as,pale,g.m);_winIcon(ctx,"left",cx-ar,cy,as,pale,g.m);_winIcon(ctx,"right",cx+ar,cy,as,pale,g.m);ctx.beginPath();ctx.arc(kx,ky,g.knobR,0,Math.PI*2);ctx.fillStyle=active?(_hexToRgba(hi,0.30)||"rgba(74,222,128,0.30)"):"rgba(255,255,255,0.10)";ctx.fill();ctx.strokeStyle=active?hi:norm;ctx.lineWidth=Math.max(1.4,g.m*0.012);ctx.beginPath();ctx.arc(kx,ky,g.knobR,0,Math.PI*2);ctx.stroke()}
function _drawFullscreenBtn(canvas,bgColor,showIcon,showLabel,iconSize){var ctx=canvas.getContext("2d"),w=canvas._logW,h=canvas._logH;ctx.clearRect(0,0,w,h);ctx.fillStyle=bgColor||"#1a3a2a";ctx.fillRect(0,0,w,h);var cx=w/2,cy=h/2;if(showIcon===undefined)showIcon=true;if(showLabel===undefined)showLabel=true;if(showIcon&&showLabel){var m=Math.min(w,h),s=iconSize||Math.round(m*0.40);var iconCY=cy-s*0.18;var textCY=cy+s*0.52;var gap=s*0.16,lw=Math.max(1.2,s*0.09);ctx.strokeStyle="rgba(255,255,255,0.8)";ctx.lineWidth=lw;ctx.beginPath();ctx.moveTo(cx-s/2,iconCY-s/2+gap);ctx.lineTo(cx-s/2,iconCY-s/2);ctx.lineTo(cx-s/2+gap,iconCY-s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx+s/2,iconCY+s/2-gap);ctx.lineTo(cx+s/2,iconCY+s/2);ctx.lineTo(cx+s/2-gap,iconCY+s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx+s/2,iconCY-s/2+gap);ctx.lineTo(cx+s/2,iconCY-s/2);ctx.lineTo(cx+s/2-gap,iconCY-s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx-s/2,iconCY+s/2-gap);ctx.lineTo(cx-s/2,iconCY+s/2);ctx.lineTo(cx-s/2+gap,iconCY+s/2);ctx.stroke();ctx.fillStyle=canvas._fontColor||"rgba(255,255,255,0.75)";var tf=Math.max(8,Math.round(Math.min(h*0.17,w*0.09)));ctx.font="bold "+tf+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("Fullscreen",cx,textCY)}else if(showIcon){var s=iconSize||Math.round(Math.min(w,h)*0.50),gap=s*0.16,lw=Math.max(1.2,s*0.10);ctx.strokeStyle="rgba(255,255,255,0.85)";ctx.lineWidth=lw;ctx.beginPath();ctx.moveTo(cx-s/2,cy-s/2+gap);ctx.lineTo(cx-s/2,cy-s/2);ctx.lineTo(cx-s/2+gap,cy-s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx+s/2,cy+s/2-gap);ctx.lineTo(cx+s/2,cy+s/2);ctx.lineTo(cx+s/2-gap,cy+s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx+s/2,cy-s/2+gap);ctx.lineTo(cx+s/2,cy-s/2);ctx.lineTo(cx+s/2-gap,cy-s/2);ctx.stroke();ctx.beginPath();ctx.moveTo(cx-s/2,cy+s/2-gap);ctx.lineTo(cx-s/2,cy+s/2);ctx.lineTo(cx-s/2+gap,cy+s/2);ctx.stroke()}else{ctx.fillStyle=canvas._fontColor||"rgba(255,255,255,0.85)";var tf=Math.max(10,Math.round(Math.min(h*0.40,w*0.16)));ctx.font="bold "+tf+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("Fullscreen",cx,cy)}}
function _drawSwitchProfileBtn(canvas,bgColor,iconColor,label,showIcon,showLabel,iconSize){var ctx=canvas.getContext("2d"),w=canvas._logW,h=canvas._logH;ctx.clearRect(0,0,w,h);ctx.fillStyle=bgColor||"#2a1a3a";ctx.fillRect(0,0,w,h);var cx=w/2,cy=h/2;if(showIcon===undefined)showIcon=true;if(showLabel===undefined)showLabel=true;var ic=iconColor||"#a78bfa";if(showIcon&&showLabel){var iconCY=cy-Math.min(h,w)*0.12;var textCY=cy+Math.min(h,w)*0.30;var as=iconSize||Math.round(Math.min(w*0.35,h*0.32));ctx.fillStyle=ic;ctx.font="bold "+as+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("⇄",cx,iconCY);ctx.fillStyle=canvas._fontColor||ic;var tf=Math.max(8,Math.round(Math.min(h*0.18,w*0.09)));ctx.font=tf+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(label||"None",cx,textCY)}else if(showIcon){var as=iconSize||Math.round(Math.min(w*0.55,h*0.50));ctx.fillStyle=ic;ctx.font="bold "+as+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("⇄",cx,cy)}else{ctx.fillStyle=canvas._fontColor||ic;var tf=Math.max(10,Math.round(Math.min(h*0.35,w*0.15)));ctx.font=tf+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText(label||"None",cx,cy)}}
function _drawTextMacroBtn(canvas,bgColor,text,fontColor,label){var ctx=canvas.getContext("2d"),w=canvas._logW,h=canvas._logH;ctx.clearRect(0,0,w,h);ctx.fillStyle=bgColor||"#1a2a1a";ctx.fillRect(0,0,w,h);var fc=fontColor||_autoFc(bgColor||"#1a2a1a"),fs=Math.max(10,Math.min(48,Math.min(w,h)*0.30));var padX=Math.max(6,w*0.05);var hdrFs=Math.max(8,fs*0.55);var topPad=Math.max(5,h*0.10);ctx.globalAlpha=0.45;ctx.fillStyle=fc;ctx.font=hdrFs+"px "+_gf();ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(label||"TEXT MACRO",padX,topPad);ctx.globalAlpha=1;var textTop=topPad+hdrFs*1.4+Math.max(2,h*0.04),textH=h-textTop-topPad;if(text&&text.length>0){var display=text;ctx.fillStyle=fc;ctx.font=fs+"px "+_gf();ctx.textAlign="left";ctx.textBaseline="top";var maxW=w-padX*2,lineH=fs*1.35,maxLines=textH>=lineH?Math.floor(textH/lineH):(textH>=fs?1:0);var lines=[];if(display.indexOf("\n")>=0){var raw=display.split("\n");for(var i=0;i<Math.min(raw.length,maxLines);i++){var ln=raw[i];if(ctx.measureText(ln).width>maxW){while(ln.length>1&&ctx.measureText(ln+"…").width>maxW)ln=ln.slice(0,-1);ln+="…"}lines.push(ln)}}else if(maxLines>0){var ln=display;if(ctx.measureText(ln).width>maxW){while(ln.length>1&&ctx.measureText(ln+"…").width>maxW)ln=ln.slice(0,-1);ln+="…"}lines.push(ln)}var totalH=lines.length*lineH;var startY=totalH<=textH?textTop+(textH-totalH)/2:textTop+2;for(var i=0;i<lines.length;i++){ctx.fillText(lines[i],padX,startY+i*lineH)}}else{ctx.globalAlpha=0.35;ctx.fillStyle=fc;ctx.font=fs+"px "+_gf();ctx.textAlign="center";ctx.textBaseline="middle";ctx.fillText("(empty)",w/2,textTop+textH/2);ctx.globalAlpha=1}}function _onWinSwipeTouch(e,canvas){e.stopPropagation();var p=_winCtrlXY(e,canvas),g=_winSwipeGeom(canvas);canvas._wsActive=true;canvas._wsSX=p.x;canvas._wsSY=p.y;canvas._wsDX=0;canvas._wsDY=0;canvas._wsHold=false;_drawWinSwipe(canvas,g.cx,g.cy,true);if(canvas._wsTimer)clearTimeout(canvas._wsTimer);canvas._wsTimer=setTimeout(function(){canvas._wsHold=true;psnd(canvas._sound||"click");fetch("/api/system/window/fullscreen",{method:"POST"}).catch(function(){});_drawWinSwipe(canvas,g.cx,g.cy,false)},500)}
...
function _drawDockGrid(canvas, apps) {
  var ctx = canvas.getContext("2d"), w = canvas._logW, h = canvas._logH;
  ...
  for (var i = 0; i < apps.length; i++) {
    ...
    ctx.fillStyle = canvas._fontColor||"#ccc"; ctx.font = fs + "px "+_gf();
...
function _onDockTouchStart(e, canvas) {
  ...
  var cx = (t.clientX - rect.left) * (canvas._logW / rect.width);
...
function _onDockTouchEnd(e, canvas) {
  ...
  var rect = canvas.getBoundingClientRect();
  var x = (t.clientX - rect.left) * (canvas._logW / rect.width);
...
let _reconnSched=false;
function _scheduleReconn(){if(_reconnSched)return;_reconnSched=true;timer=setTimeout(function(){_reconnSched=false;delay=Math.min(delay*1.5,15000);conn()},delay)}
