...
function _icThumb(){return '<label>&#x263A; Icon</label><canvas class="icThumb" width="68" height="68" style="width:34px;height:34px;flex:0 0 auto;display:block;margin:2px 0 6px;background:var(--bg);border:1px solid var(--border);border-radius:4px"></canvas>'}function _drawThumbs(){if(!selKey)return;var src=null,srcEl=null,k=null;var els=document.querySelectorAll(".ck");for(var i=0;i<els.length;i++){if(els[i].getAttribute("data-kid")===selKey){srcEl=els[i];src=els[i].querySelector("canvas");break}}var p=cp_();if(p&&p.keys){for(var j=0;j<p.keys.length;j++){if(p.keys[j].id===selKey){k=p.keys[j];break}}}var ts=document.querySelectorAll("#pc .icThumb");for(var t=0;t<ts.length;t++){var c=ts[t].getContext("2d");c.clearRect(0,0,68,68);if(k&&k.color&&k.color.charAt(0)==="#"){c.fillStyle=k.color;c.fillRect(0,0,68,68)}if(src&&src.width>0&&src.height>0){var s2=Math.min(60/src.width,60/src.height),w2=src.width*s2,h2=src.height*s2;c.drawImage(src,(68-w2)/2,(68-h2)/2,w2,h2)}else if(k){c.fillStyle=k.fontColor||"#e8e0d8";c.font="bold 17px "+_gf();c.textAlign="center";c.textBaseline="middle";c.fillText(k.label||"Key",34,34)}}}
