...
const ftb=document.getElementById("ftb");var btb=document.getElementById("btb");if(btb){var wr=wrap.getBoundingClientRect();var cr=ca.getBoundingClientRect();btb.style.left="50%";btb.style.transform="translateX(-50%)";btb.style.top=(wr.bottom-cr.top+8)+"px";}ftb.style.right=ox+"px";ftb.style.top=Math.max(0,oy-36)+"px";ftb.style.left="auto";
  ...
  page.keys.forEach(k=>{
    ...
    if(k.action==='balance'){var _pv=document.createElement('canvas');_pv.width=kw-8;_pv.height=kh-8;_pv.style.borderRadius='3px';var _pctx=_pv.getContext('2d');_pctx.fillStyle='rgba(0,0,0,0.3)';_pctx.fillRect(0,0,_pv.width,_pv.height);_pctx.fillStyle='#f59e0b';var _pfs=Math.max(9,Math.min(kh*0.3,_pv.width*0.06));_pctx.font='bold '+_pfs+'px -apple-system,sans-serif';_pctx.textAlign='center';_pctx.textBaseline='middle';_pctx.fillText(k.apiKey?'Deepseek ¥ ???':'Set API Key',_pv.width/2,_pv.height/2);el.appendChild(_pv)}else{el.innerHTML='<span class="kl">'+hesc(k.label)+'</span>'+(w>1||h>1?'<span class="sz">'+w.toFixed(2)+'×'+h.toFixed(2)+'</span>':'')+((selKeys.size<=1?'<div class="rh"></div>':''))}
