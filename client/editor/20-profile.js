...
async function lp(fn){if(fn==="__manage__"){openProfileManager();return}const r=await fetch("/api/profiles/"+fn);if(!r.ok){if(profiles&&profiles.length>0){return lp(profiles[0].filename)}else{activeProfile="";profile=null;profileLoaded=true;selKey=null;selKeys.clear();try{localStorage.removeItem("stp_active")}catch(e){};renderAll();return}}profile=await r.json();activeProfile=fn;activePage=profile.pages[0]?.id||"";selKey=null;selKeys.clear();panX=profile.canvasX||0;panY=profile.canvasY||0;viewX=0;viewY=0;viewZoom=Math.max(0.3,Math.min(3,profile.viewZoom||1));try{localStorage.setItem("stp_active",fn)}catch(e){};profileLoaded=true;renderAll();fetch("/api/active-profile",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filename:fn})}).catch(function(){})}
...
async function dp(fn){if(!confirm("Delete "+fn+"?"))return;await fetch("/api/profiles/"+fn,{method:"DELETE"});if(activeProfile===fn){var rem=profiles.filter(function(p){return p.filename!==fn});if(rem.length>0){await lp(rem[0].filename)}else{activeProfile="";profile=null;profileLoaded=false;selKey=null;selKeys.clear();try{localStorage.removeItem("stp_active")}catch(e){}}}await lpl();renderAll()}
